...
A note on models: this used Imagen 4 Fast and Gemini 2.5 Flash until August 2026, when both stopped being served to newly issued API keys and started returning 404. It now uses Gemini 3.1 Flash Lite Image and Gemini 3.5 Flash Lite, which also lifted the image quota from 70 a day to 1000. I compared the candidates on eight dishes before picking: the lite models matched the larger ones on output I could tell apart, and ran in roughly half the time.

![Python](https://img.shields.io/badge/Python-3.8+-green) ![Streamlit](https://img.shields.io/badge/Streamlit-1.28+-red)

## Features

- **Menu reading**: Gemini 3.5 Flash Lite does the OCR and extracts each dish's name, description and price
- **Image generation**: Gemini 3.1 Flash Lite Image generates one photo per dish
...
- **OCR & Menu Parsing**: [Google Gemini 3.5 Flash Lite](https://deepmind.google/technologies/gemini/) via `google-genai` SDK
- **Image Generation**: [Google Gemini 3.1 Flash Lite Image](https://deepmind.google/technologies/gemini/) via `google-genai` SDK
- **Language**: Python 3.8+
...
3. **Generate**: Gemini 3.1 Flash Lite Image generates a photo for each dish, concurrently
